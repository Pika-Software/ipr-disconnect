...
hook.Add( "PlayerDisconnected", "PlayerDataSaving", function( ply )
    ...
    entity.Nick = ply:Nick()
    entity.IsBot = isBot

    if not isBot then
        entity.SteamID64 = ply:SteamID64()
    end

    ragdolls[ #ragdolls + 1 ] = entity
    entity.Alive = true
...
hook.Add( "PreCleanupMap", "Cleanup", function()
    table.Empty( ragdolls )
end )

hook.Add( "EntityRemoved", "Removed", function( entity )
    for index, ragdollEntity in ipairs( ragdolls ) do
        if ragdollEntity == entity then
            table.remove( ragdolls, index )
        end
    end
end )
...
local function findRagdoll( ply )
    local isBot = ply:IsBot()
    local sid = isBot and ply:Nick() or ply:SteamID64()

    for index, entity in ipairs( ragdolls ) do
        if not IsValid( entity ) then continue end
        if entity.IsBot then
            if entity.Nick == sid then
                table.remove( ragdolls, index )
                return entity
            end
        elseif entity.SteamID64 == sid then
            table.remove( ragdolls, index )
            return entity
        end
    end
